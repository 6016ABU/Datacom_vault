...
2.设备类型：  
&emsp;1.LER（Label Edge Router）：边界设备  
&emsp;&emsp;1.ingress：入节点，设备对应的标签处理动作为push（压入标签）  
&emsp;&emsp;2.egress：出节点，设备对应的标签处理动作为pop（弹出标签） 

&emsp;2.LSR（Label Switching Router）：内部设备  
&emsp;&emsp;transit：中间节点，设备对应的标签处理动作为swap（替换标签）

&emsp;3.LSP：（Label Switched Path，标签交换路径）  
&emsp;&emsp;由一个入节点 到 一个出节点 组成一条转发路径  
&emsp;&emsp;该路径上可以存在 0个 或 多个 中间节点  


**判断LSP的方向 可以根据上下游设备来判断**&emsp;

&emsp;ingress 是 egress 的 上游节点  
&emsp;egress 是 ingress 的 下游节点  
&emsp;ingress 是 transit的 上游节点  
&emsp;transit 是 egress 的 上游节点  

4.MPLS体系结构：
![837](assets/1、MPLS概论/file-20260519155351681.png)
5.FEC：（Forwarding Equivalence Class，转发等价类）  
&emsp;&emsp;MPLS对于某些特征相同的报文 并 执行相同的处理动作，称为FEC  
&emsp;&emsp;FEC可以通过SIP、DIP、Sport、Dport、VPN等要素来判断是否为相同特征的报文  
**对于认为相同的流量，有统一的处理动作**
...
**MPLS报文头部一共是32bit长度**  
报文格式：
![600](assets/1、MPLS概论/file-20260519155351672.png)
### 报文格式的内容：  
**1.标签值域（第0-20bit）：标签的取值范围是多少  1048576**<br>
&emsp;1.特殊标签值：取值范围是0-15  
&emsp;&emsp;0号标签：显示空标签，在倒数第二跳设备发送数据时，会保留标签给到最后一跳设备，该功能主要用于保证qos数据优先转发的操作<br>
&emsp;&emsp;3号标签：隐式空标签，在倒数第二跳设备发送数据时，会直接执行数据的标签弹出（默认的功能）<br>
&emsp;&emsp;1号标签：在最外层，表示收到1号标签按照下一层标签的处理进行转发，转发时还要封装1号标签<br>
&emsp;&emsp;2号标签：显示空标签，和0号标签的作用类型，用于IPv6网络<br>
&emsp;&emsp;14号标签：通过发送OAM报文检测和通告LSP故障，告知LSP路径上的所有设备<br>
&emsp;2.静态标签值：取值范围是16--1023，管理员手工配置。<br>
&emsp;3.动态标签值：取值范围是1024--2^20，一般通过动态获取标签。<br>
&emsp;&emsp;1.LDP协议<br>
&emsp;&emsp;2.MP-BGP协议<br>
&emsp;&emsp;3.RSVT-TE协议（属于策略分配）<br>

 
**2.exp（第21-23bit）（Experimental Use）：优先级的取值**<br>
3bit 值是0-7 给QoS调度服务时使用 000-111  <br>
==该值的使用，只能在MPLS域中==
 
**3.S （Bottom of Stack）（第24bit）：栈低标签位**<br>
==MPLS是可以支持多层嵌套的==<br>
S bit 置 0：表示MPLS头部不是靠近IP报文头<br>
S bit 置 1：表示MPLS头部紧挨着IP报文头<br>
 
**4.TTL（第25-32bit）：和IP的TTL值作用相同**<br>
标签栈：多个MPLS标签头部的集合，称之为标签栈  
标签栈的第一个头部（靠近数据链路层头部的MPLS头部），称为栈顶标签<br>
标签栈的最后一个头部（靠近IP层的MPLS头部），称为栈低标签

## 静态MPLS LSP配置思路：  
1.设备接口地址配置完成（AR1、AR5存在环回口地址）  
2.构建AR1-AR5的MPLS静态隧道完成环回口通信
（MPLS是直接通过标签执行数据转发的，设备如果存在标签值，但是不存在路由，设备可以直接通过标签值转发数据。）<br>
ingress只存在出标签，没有入标签  
egress只存在入标签，没有出标签
```d
...
```
![900](assets/1、MPLS概论/file-20260519155351665.png)
...
3.查看静态mpls-lsp
![](assets/1、MPLS概论/file-20260519155351641.png) ![](assets/1、MPLS概论/file-20260519155351628.png)
