...
![](assets/8、MPLS%20VPN（OSPF站点双归,MPE和MCE）/file-20260508103456037%201.png)
...
![500](assets/8、MPLS%20VPN（OSPF站点双归,MPE和MCE）/file-20260508103456037.png)
**DN bit的作用：**  
...
![](assets/8、MPLS%20VPN（OSPF站点双归,MPE和MCE）/file-20260508103456035.png) 
![](assets/8、MPLS%20VPN（OSPF站点双归,MPE和MCE）/file-20260508103456013.png)
![700](assets/8、MPLS%20VPN（OSPF站点双归,MPE和MCE）/file-20260508103456011.png)
...
![](assets/8、MPLS%20VPN（OSPF站点双归,MPE和MCE）/file-20260508103456012.png)
```D
...
## MCE：多CE场景  
MCE（Multi-VPN-Instance CE，多 VPN 实例 CE，也叫 VRF-Lite），是 MPLS L3VPN 中对 CE 设备的功能增强。其核心能力是**在单台 CE 设备上创建多个相互隔离的 VPN 实例（VRF）**，每个 VRF 对应骨干网中一个独立的 VPN，实现单台 CE 同时接入多个 VPN 业务，且不同 VPN 的路由、转发全程完全隔离。
...
```
**配置影响**
- 在MCE上配置**vpn-instance-capability simple**命令后，如果OSPF没有配置骨干区域0，则该MCE不会成为ABR。
- 配置**vpn-instance-capability simple**命令后，OSPF进程不可以引入IBGP路由。
- 配置**vpn-instance-capability simple**命令后，BGP引入的OSPF路由中不会携带OSPF Domain ID、OSPF Route-tag和OSPF Router ID。
- 缺省情况下，当BGP引入OSPF路由时，MED值（MED属性相当于IGP使用的度量值）为OSPF的Cost值加1。配置**vpn-instance-capability simple**命令后，Cost值不会加1，即MED值变为OSPF的Cost值。因此，会引起BGP引入OSPF路由的MED值变化，影响BGP选路。