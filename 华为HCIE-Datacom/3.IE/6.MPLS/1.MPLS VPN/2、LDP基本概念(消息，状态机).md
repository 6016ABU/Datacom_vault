...
## **LDP：label Distribute protocol标签分发协议**
LDP的作用：就是在LSR之间建立LDP会话，然后向邻居通告标签和FEC（转发等价类）信息，以此来达到为路由动态分配标签并建立LSP的目的 
**LDP的邻接体：**
==收到对方发送的hello报文就认为对方是自己的邻接体==  
	1.本地邻接体：就是直连的设备之间通过==组播==发送hello消息  
	2.远端邻接体：就是非直连的设备之间通过==单播==发送hello消息
...
**LDP的4大消息类型：**  
	...
	3.advertise  
	4.notification
![](assets/2、LDP基本概念(消息，状态机)/file-20260519155352473.png)
...
```
将hello报文通告的传输地址替换为 loopback 2接口的地址   

### **TCP连接建立：**
当收到邻居的hello消息后，根据携带的传输地址，判断其是否可达  
如果地址可达，则由==传输地址大的一方发起TCP连接建立==（TCP报文都是单播的，源目IP地址为双方的传输地址）  
...
报文信息
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352487.png)

**2.keepalive：维护邻居之间的会话**
收到邻居的init消息后，如果参数协商没有问题，则回复keepalive报文并发送init报文与对方协商  
报文信息  
对于keepalive消息周期15s发送，45s超时
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352480.png)
## **3.advertis message：通告消息**
**1.address 消息**
==由传输地址大的一方 回复 地址小的一方一个keepalive报文 同时发送一个address消息  ==
address消息内容：  
1.通告地址
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352500.png)  

2.撤销地址
![](assets/2、LDP基本概念(消息，状态机)/file-20260519155352493.png)  

**2.Label 消息**
LSR之间将自身所有的映射消息同步给邻居  
1.lable mapping：宣告FEC/标签映射信息
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352521.png)

2.lable withdrawal：撤销FEC/标签映射信息
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352514.png)

3.lable release：释放FEC/标签映射信息
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352527.png)


4.lable request：请求FEC/标签映射信息
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352507.png)

5.lable abrot request：终止未完成的标签请求
![](assets/2、LDP基本概念(消息，状态机)/file-20260519155352535.png)

## 4.notification message：通知消息  
用于提供差错消息的
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352541.png)

## LDP的状态机：
![900](assets/2、LDP基本概念(消息，状态机)/file-20260519155352548.png)

==在任何状态下收到通知消息或该状态的定时器超时，都会进入non-existent（重新建立TCP的连接）==
![](assets/2、LDP基本概念(消息，状态机)/file-20260519155352471.png)
