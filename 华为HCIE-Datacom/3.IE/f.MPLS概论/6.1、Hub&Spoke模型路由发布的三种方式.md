...
## 1.站点和Hub都是BGP
![](assets/6.1、Hub&Spoke模型路由发布的三种方式/file-20260508100930068.png)  
## 2.站点和Hub都是OSPF
![](assets/6.1、Hub&Spoke模型路由发布的三种方式/file-20260508100930069.png)
...
配置影响
1.在MCE上配置vpn-instance-capability simple命令后，如果OSPF没有配置骨干区域0，则该MCE不会成为ABR。
2.配置vpn-instance-capability simple命令后，OSPF进程不可以引入IBGP路由。
3.配置vpn-instance-capability simple命令后，BGP引入的OSPF路由中不会携带OSPF Domain ID、OSPF Route-tag和OSPF Router ID。
4.缺省情况下，当BGP引入OSPF路由时，MED值（MED属性相当于IGP使用的度量值）为OSPF的Cost值加1。配置vpn-instance-capability simple命令后，Cost值不会加1，即MED值变为OSPF的Cost值。因此，会引起BGP引入OSPF路由的MED值变化，影响BGP选路。
```
4. 在BGP ipv4-family vpn-instance out 地址族下，引入OSPF 200的路由，将路由发送出去。

## 3.站点OSPF，Hub-BGP
![](assets/6.1、Hub&Spoke模型路由发布的三种方式/file-20260508100930072.png)
## 存在问题的第四种方式：
==IGP再引入BGP路由时，会忽略AS_Path。==
![](assets/6.1、Hub&Spoke模型路由发布的三种方式/file-20260508100930071.png)
