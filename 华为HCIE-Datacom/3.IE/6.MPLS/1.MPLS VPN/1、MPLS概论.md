...
&emsp;&emsp;&emsp;1.ingress：入节点，设备对应的标签处理动作为push（压入标签）  
&emsp;&emsp;&emsp;2.egress：出节点，设备对应的标签处理动作为pop（弹出标签）  
&emsp;2.LSR（Label Switching Router）：内部设备  
&emsp;&emsp;&emsp;transit：中间节点，设备对应的标签处理动作为swap（替换标签）
&emsp;3.LSP：（Label Switched Path，标签交换路径）  
&emsp;&emsp;&emsp;由一个入节点 到 一个出节点 组成一条转发路径  
&emsp;&emsp;&emsp;该路径上可以存在 0个 或 多个 中间节点  
...
### 报文格式的内容：  
**1.标签值域（第0-20bit）：标签的取值范围是多少  1048576**
	1.特殊标签值：取值范围是0-15  
