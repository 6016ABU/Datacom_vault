...
| 分类标准名称（中文 / 英文）                              | 协议规范核心定义                                                                                    | 标准归属属性（规范写法）                                                               | 关键约束与避坑要点                                                                                                                      |
| :------------------------------------------- | :------------------------------------------------------------------------------------------ | :------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------- |
| 公认必遵<br><br>（Well-known Mandatory）           | 所有 BGP 实现必须能够识别该类属性；**在发布可达路由（NLRI）的 Update 报文中，必须完整携带该类属性**，路由撤销报文无需携带                     | `AS_PATH`、`ORIGIN`、`NEXT_HOP`                                              | 是 BGP 可达路由 Update 报文的强制必带属性，缺少任一属性均视为错误报文，对等体将直接拒收                                                                             |
| 公认任意（公认自决）<br><br>（Well-known Discretionary） | 所有 BGP 实现必须能够识别该类属性；可根据路由策略与场景需求，选择是否在 Update 报文中携带；**一旦携带该属性，必须按照协议规范正确处理**，不得随意忽略         | `LOCAL_PREF`（全称 Local_Preference，本地优先级）、`ATOMIC_AGGREGATE`（原子聚合）           | `LOCAL_PREF`仅允许在 IBGP 邻居间传递，禁止发送给 EBGP 邻居；`ATOMIC_AGGREGATE`用于标记路由聚合，携带后不可再拆分聚合路由                                              |
| 可选过渡<br><br>（Optional Transitive）            | BGP 实现可以不支持 / 不识别该类属性；**若收到不识别的可选过渡属性，必须完整保留该属性，且无条件传递给其他 BGP 邻居**；若识别该属性，按协议规范处理后可自主决定是否传递 | `AGGREGATOR`（聚合者）、`COMMUNITY`（团体属性）                                        | 核心纠正：“过渡” 是对不识别属性的**强制传递要求**，而非 “可以传递”；`COMMUNITY`为扩展团体属性的基础，常用于路由策略的批量匹配                                                      |
| 可选非过渡<br><br>（Optional Non-transitive）       | BGP 实现可以不支持 / 不识别该类属性；**若收到不识别的可选非过渡属性，必须直接忽略并丢弃，且不得传递给任何 BGP 邻居**；若识别该属性，可按协议规范在限定范围内处理与传递 | `MULTI_EXIT_DISC`（MED，多出口鉴别器）、`ORIGINATOR_ID`（始发者 ID）、`CLUSTER_LIST`（集群列表） | MED 仅可传递给直连 EBGP 邻居，仅能在本地 AS 内 IBGP 间传递，不得转发给其他 AS；`ORIGINATOR_ID`与`CLUSTER_LIST`为 BGP 路由反射器（RR）专用属性，仅在 IBGP 域内传递，禁止发给 EBGP 邻居 |
...
```
  ...
  [AR7-route-policy]apply origin egp 123   #123 标识来源EGP 的AS号，为什么需要，因为本网络都是BGP，所有需要标识egp的来源。  
