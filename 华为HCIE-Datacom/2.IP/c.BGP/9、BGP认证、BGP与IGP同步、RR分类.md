...
```
   ![650](assets/9、BGP认证、BGP与IGP同步、RR分类/file-20260506174744735.png)


### BGP与IGP的同步：  
问题由来：设备的IGP邻居 一定是 优于 BGP邻居先建立的；设备一定是先学习了IGP的路由 才会 学习BGP的路由  
当主用路径的设备重启后，因为IGP建立优于BGP，所以可能会导致BGP的路由黑洞
（AR2重启没建立IBGP邻居，会导致找不到路）

OSFP：
通过设置
```
[AR2-ospf-1]stub-router on-startup 100 # OSPF协议将设备配置为末节路由器（即将IGP的路径开销设置为最大值65535）
```
ISIS：
通过设置
```
AR2-isis-1]set-overload on-startup wait-for-bgp 100 # ISIS协议将设备作为末节设备设置最大值的开销
```
