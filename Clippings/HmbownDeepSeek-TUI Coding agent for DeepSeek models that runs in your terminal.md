...
> 终端编码代理，适用于 DeepSeek V4。它从 `deepseek` 命令运行，流式传输推理块，使用批准门编辑本地工作空间，并包括一个自动模式，每轮选择模型和思维水平。

[简体中文 README](https://github.com/Hmbown/DeepSeek-TUI/blob/main/README.zh-CN.md)
...
`deepseek` 以 Rust 二进制文件形式分发：调度程序命令（ `deepseek` ）和配套的 TUI 运行时（ `deepseek-tui` ）。选择您已经使用的安装路径；它们都会在您的 `PATH` 上放置相同的命令。npm 包是用于发布二进制文件的安装程序/包装器，而不是代理运行时本身。
...
> In mainland China, speed up the npm path with `--registry=https://registry.npmmirror.com`, or use the [Cargo mirror](#china--mirror-friendly-installation) below.  
> 在中国大陆，使用 `--registry=https://registry.npmmirror.com` 加快 npm 路径，或使用下方的 Cargo 镜像。
...
DeepSeek TUI 是一个在终端中运行的编码代理。它可以读取和编辑文件、运行 shell 命令、搜索网页、管理 git，并通过键盘驱动的 TUI 协调子代理。
...
See [docs/ARCHITECTURE.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/ARCHITECTURE.md) for the full walkthrough.
...
Prebuilt binaries are published for **Linux x64**, **Linux ARM64** (v0.8.8+), **macOS x64**, **macOS ARM64**, and **Windows x64**. For other targets (musl, riscv64, FreeBSD, etc.), see [Install from source](#install-from-source) or [docs/INSTALL.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/INSTALL.md).
...
Both binaries are required. Cross-compilation and platform-specific notes: [docs/INSTALL.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/INSTALL.md).
...
A stabilization release focused on first-run setup, auto model + thinking routing, cost accounting, and provider support. [Full changelog](https://github.com/Hmbown/DeepSeek-TUI/blob/main/CHANGELOG.md).
...
Full shortcut catalog: [docs/KEYBINDINGS.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/KEYBINDINGS.md).
...
User config: `~/.deepseek/config.toml`. Project overlay: `<workspace>/.deepseek/config.toml` (denied: `api_key`, `base_url`, `provider`, `mcp_config_path`). [config.example.toml](https://github.com/Hmbown/DeepSeek-TUI/blob/main/config.example.toml) has every option.
...
UI locale is separate from model language — set `locale` in `settings.toml`, use `/config locale zh-Hans`, or rely on `LC_ALL` / `LANG`. See [docs/CONFIGURATION.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/CONFIGURATION.md) and [docs/MCP.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/MCP.md).
...
| Doc | Topic |
| --- | --- |
| [ARCHITECTURE.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/ARCHITECTURE.md) | Codebase internals |
| [CONFIGURATION.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/CONFIGURATION.md) | Full config reference |
| [MODES.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/MODES.md) | Plan / Agent / YOLO modes |
| [MCP.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/MCP.md) | Model Context Protocol integration |
| [RUNTIME\_API.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/RUNTIME_API.md) | HTTP/SSE API server |
| [INSTALL.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/INSTALL.md) | Platform-specific install guide |
| [MEMORY.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/MEMORY.md) | User memory feature guide |
| [SUBAGENTS.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/SUBAGENTS.md) | Sub-agent role taxonomy and lifecycle |
| [KEYBINDINGS.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/KEYBINDINGS.md) | Full shortcut catalog |
| [RELEASE\_RUNBOOK.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/RELEASE_RUNBOOK.md) | Release process |
| [LOCALIZATION.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/LOCALIZATION.md) | UI locale matrix & switching |
| [OPERATIONS\_RUNBOOK.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/docs/OPERATIONS_RUNBOOK.md) | Ops & recovery |

Full Changelog: [CHANGELOG.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/CHANGELOG.md).
...
- **[zichen0116](https://github.com/zichen0116)** — CODE\_OF\_CONDUCT.md (#686)
...
See [CONTRIBUTING.md](https://github.com/Hmbown/DeepSeek-TUI/blob/main/CONTRIBUTING.md). Pull requests welcome — check the [open issues](https://github.com/Hmbown/DeepSeek-TUI/issues) for good first contributions.
...
[MIT](https://github.com/Hmbown/DeepSeek-TUI/blob/main/LICENSE)

[![Star History Chart](https://camo.githubusercontent.com/cb67f076ea2e857112491b340c2bbdbca932cf02e526cbb4d87ba04e9c30be60/68747470733a2f2f6170692e737461722d686973746f72792e636f6d2f63686172743f7265706f733d486d626f776e2f446565705365656b2d54554926747970653d64617465266c6567656e643d746f702d6c656674)](https://www.star-history.com/?repos=Hmbown%2FDeepSeek-TUI&type=date&logscale=&legend=top-left)