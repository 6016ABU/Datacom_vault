...
![](assets/4.路径建立/file-20260519155509264.png)

**MPLS RSVP-TE(信令)协议——对象类型(1)**
![](assets/4.路径建立/file-20260519155509263.png)
**MPLS RSVP-TE(信令)协议——对象类型(2)**
![](assets/4.路径建立/file-20260519155509261.png)

**MPLS RSVP-TE(信令)协议一报文格式**
- RSVP消息在IP报头中的==协议类型号为46==，每个RSVP消息都包含一个通用的RSVP头部，随后是一个或者多个可变长度、类型的消息对象(objects)
![](assets/4.路径建立/file-20260519155509207.png)

**MPLS RSVP-TE(信令)协议——报文格式**
![](assets/4.路径建立/file-20260519155509207%201.png)
...
![](assets/4.路径建立/file-20260519155509171.png)
...
![](assets/4.路径建立/file-20260519155509142.png)
...
**RSVP-TE建立CR-LSP的流程**
1. **Ingress**触发CSPF计算从**Ingress**到**Egress**的路径，这条路径是指定了沿途每一跳的IP地址。**Ingress**将CSPF计算出来的IP地址列表作为对象ERO(Explicit Route Object)的内容，构造Path消息，并根据Path消息构造PSB(Path State Block，路径状态块)，然后根据ERO将Path消息发送给**Transit**
2. **Transit**收到**Ingress**的Path消息，解析报文，根据Path消息构造PSB。然后**Transit**==更新Path消息==，根据ERO将Path消息发送给**Egress**
	- 与**Ingress**更新Path消息的RSVP_HOP为**Ingress**到**Transit**的出接口地址类似**Transit**更新Path消息的RSVP_HOP为**Transit**到**Egress**的出接口地址
	- **Transit**更新Path消息的ERO，删除**Transit**自己的出、入接口地址及LSR-ID。
3. **Egress**收到Path消息后，从Session对象的Destination内容获知自己为待建立的CR-LSP的**Egress**节点。此时，**Egress**分配标签和资源，根据本地PSB产生Resv消息。Resv消息被发送给**Transit**，该消息携带了**Egress**分配给**Transit**的标签
	- **Egress**从收到的Path消息中提取RSVP_HOP字段的地址作为Resv消息的目的IP地址。此外，Resv消息沿着反向路径进行转发，因此Resv消息中并不携带ERO
	- Resv消息如果包含RESV_CONFIRM对象，接收该消息的节点需要向发送该消息的节点发送ResvConf消息来确认资源预留请求
4. 当**Transit**收到Resv消息时，记录相关信息到RSB中，并分配一个新标签，更新Resv消息发给**Ingress**
5. **Ingress**收到Resv消息，获得**Transit**分配的标签，表明资源预留成功，此时一条单向(**Ingress**->**Egress**方向)的CR-LSP建立成功
